        ...
        return <>
            {pedido.map(item => {
                if (item.cliente._id === user._id) {
                    const categoria = item.produto.categoriaProduto as any;
                    const obj = Object.keys(categoria)[0].toString();
                    console.log(item)

                    return <>
                        <tr>
                            <th><img src={categoria[obj].imagemProduto[0].url} alt="iamgem do produto" width="100" /></th>
                            <th>{item.quantidadePedido}</th>
                            <th>{item.total}</th>
                            <button className="btn-exclui" onClick={() => cancelarPedido(item._id)}>Cancelar Pedido</button>
                        </tr>
