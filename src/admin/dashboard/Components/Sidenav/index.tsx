...
const DashboardSidenav = () => {
	...
	function MostrarImagemPerfil() {
		if (user.imagemPerfil === null || user.imagemPerfil === undefined) {
			...
			return (
				<div
					className={`${
						collapsed ? 'h-[50px] w-[50px]' : 'h-[80px] w-[80px]'
					} pfp-container`}
				>
	...
	return (
		<>
			<div
				className={`${
					collapsed ? 'setCollapsedFalse' : 'setCollapsedTrue'
				} sidenav-container`}
			>
				<div
					className={`${
						collapsed ? 'sidenav-collapsed' : 'sidenav-open'
					} sidenav`}
				>
					...
					<ul className="nav-list">
						...
						{arrayNavCadastros.map((item, index) => (
							<SubMenu item={item} key={`SubMenu-${index}`} />
						))}
						<span className="sidenav-title">Gerenciar Perfis</span>
						{arrayNavPerfis.map((item, index) => (
							<SubMenu item={item} key={`SubMenu-${index}`} />
						))}
					</ul>
				</div>
